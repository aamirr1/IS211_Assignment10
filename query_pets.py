...
while personID != -1:
	...
	print "What is the Person ID Number? (Enter '-1' to Exit)"
	personID = raw_input()
	...
	try:
		...
		print "Not valid integer ID, please try again"
		continue
	if personID == -1:
		sys.exit(1)
	...
	if data is not None:
		...
		for row in pet_data:
			if row[3] == 1:
				...
				tense = ("owns","is")
			print "%s %s %s %s, a %s, that %s %d years old" % (data[0],data[1],tense[0],row[0],row[1],tense[1],row[2])
		else:
